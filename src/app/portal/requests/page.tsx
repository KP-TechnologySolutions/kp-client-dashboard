...
import { getRequestsForOrg, getProfile } from "@/lib/mock-data";
...
export default async function PortalRequestsPage() {
  ...
  const allRequests = getRequestsForOrg(user.organization_id).sort(
    (a, b) =>
      new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
  );

  // Pin active requests to top
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {allRequests.length} total request
      ...
      {allRequests.length === 0 ? (
        <EmptyState
          ...
          description="Submit your first request and we'll get working on it."
          action={
            <Link href="/portal/requests/new" className={buttonVariants()}>
              Submit a Request
            </Link>
...
function RequestCard({
  ...
  req: ReturnType<typeof getRequestsForOrg>[number];
}) {
  const assignee = req.assigned_to ? getProfile(req.assigned_to) : null;
